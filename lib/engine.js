...
//
// Pure and offline: no network, no Agent calls, deterministic. Adapters (M2) are injected via
...
export function runToCompletion(state, deps = {}) {
  let s = state;
  // Resume from a suspended status: 'blocked' under an escape hatch (force),
  // 'awaiting-confirm' under confirmation (assist mode). Symmetric, so neither status can deadlock.
  if (s.status === 'blocked' && deps.force) s = advance(s, deps);
  if (s.status === 'awaiting-confirm' && deps.confirmed) s = advance(s, deps);
  while (s.status === 'running') s = advance(s, deps);
  return s;
}

/**
 * Full uninterrupted run — autopilot, no gate, default adapters. A convenience wrapper over
 * createRun + runToCompletion. For gating / assist mode / custom adapters, use createRun(intent,
 * opts) + runToCompletion(state, deps) directly.
 * @param {string} intent
